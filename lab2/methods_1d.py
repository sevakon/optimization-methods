...
def minimize_1d(
    function: Union[Callable, Tuple[Callable, Callable]],
    method: str, a: float, b: float, e: float,
    ...
    if isinstance(function, tuple) or isinstance(function, list):
        # fn + its derivative
        _check_derivative_method_name(method)
        
        x_min, y_min, num_calcs = _NAME_TO_FN[method](
            function[0], function[1], a, b, e)

        x = np.linspace(a, b, _NUM_POINTS)
        y = list(map(function[0], x))
...
def plot_log_e_to_num_iters(
    function: Union[Callable, Tuple[Callable, Callable]], 
    method: str, a: float, b: float, title: str, 
):
    """ Plots log e to num iters graph """
    
    epsilons = np.linspace(_MIN_E, _MAX_E, 100)
    calcs = list()
    ...
    if isinstance(function, tuple) or isinstance(function, list):
        ...
        for e in epsilons:
            _, _, num_calcs = _NAME_TO_FN[method](
                function[0], function[1], a, b, e)
...
def _brent_deriv_min(f, df, a, b, eps):    
    num_calcs = 0
    ...
    while True:
        ...
        if x != w and df_x != df_w:
            u1 = min_by_secant(x, w, df_x, df_w)
                
            if not (u1 >= a + eps and u1 <= b - eps and abs(u1 - x) < g / 2): 
                u1 = None
                
        u2 = None
        if x != v and df_x != df_v:
            u2 = min_by_secant(x, v, df_x, df_v)
            
            if not (u2 >= a + eps and u2 <= b - eps and abs(u2 - x) < g / 2):
                u2 = None
        
        if u1 is not None and u2 is not None:
            u = u1 if abs(u1 - x) < abs(u2 - x) else u2
        elif u1 is None and u2 is None:
            u = (a + x) / 2 if df_x > 0 else (x + b) / 2
        else:
            u = u1 if u1 is not None else u2            
...
_NAME_TO_FN = {
    ...
    'brent':_brent_min,
    'brent_derivative':_brent_deriv_min
}
...
def _check_derivative_method_name(method_name: str):
    if method_name not in _AVAILABLE_METHODS_DERIVATIVE:
        raise ValueError(
            f"{method_name} not supported. \n" +
            f"Choose one of {_AVAILABLE_METHODS_DERIVATIVE}")    
